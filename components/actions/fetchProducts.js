...
export async function fetchProducts(page = 1, userId) {
    ...
    const datas = fetch(`${apiEndpoint}?page=${page}&userId=${userId}`, { next: { revalidate: 0 } }).then((res) =>
        res.json()
    )
    return datas
}
...
export async function fetchProduct(id, userId) {
    ...
    const datas = fetch(`${apiEndpoint}/${id}?userId=${userId}`, { next: { revalidate: 0 } }).then((res) => res.json())
    return datas
}

export const fetchFeaturedProducts = async (userId) => {
    //const datas = await getFeaturedProducts(userId)
    const datas = fetch(`${apiEndpoint}/featured?userId=${userId}`, { next: { revalidate: 0 } }).then((res) =>
        res.json()
    )
    return datas
}
